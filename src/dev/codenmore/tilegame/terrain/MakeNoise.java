...
class MakeNoise implements Runnable{
    ...
    public static void sumOctaves(int numberOfIterations,int x,int y,double persistence,double scale,int low,int high,int k,int j)
    {
        double maxAmp=0;
        double amp=10;
    ...
    public static double[][] init(int width,int height,String name,double sc,int low,int high,int seed,int x,int y)
    {
        ...
        for(int i=0 ; i<width ; i++)
        {
            for(int j=0 ; j<height ; j++) {
                /*opennoise[j][i] = */sumOctaves(16,i+x,j+y,0.5,scale,low,high,i,j);
            }
        }
